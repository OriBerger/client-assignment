...
function DataTable({ columns, data }) {
    ...
    const visibleColumnsArray = useMemo(() => columns
    ...
    <div className="data-table-container">
        <div className="table-controls">
            <div className="filter-checkboxes">
                {columns.map(col => (
                    <label key={col.id}>
                        <input type="checkbox" checked={visibleColumns[col.id]} onChange={() => toggleColumn(col.id)} />
                        {col.title}
                    </label>
                ))}
            </div>
      ...
      <div className="pagination">
        <div className="pagination-rows-per-page">
          ...
          <span className="total-rows">
            Showing {startIndex + 1} - {Math.min(endIndex, tableData.length)} of {tableData.length} rows
          </span>
        </div>
